...
這樣的優勢非常明顯，一來程式易懂好維護 (想想下一個接手的人)，二來要修改也很簡單。重點是程式碼從組合碎片式的資訊轉而關注**程式的行為能力**。

但這時可能有人就會有疑問...**封裝的好處我懂，但如果濫用了程式碼一樣亂啊！**

### 配合業務語言封裝程式的行為

DDD 注重將業務語言注入程式模型之中，所以**只會對重點業務行為進行封裝**。如此一來，程式模型與業務邏輯綁在一起，不但可以緊隨業務的變化做修改，而且利用業務語言做封裝也解決了程式設計師的[兩大難題](https://martinfowler.com/bliki/TwoHardThings.html)之一：命名。
